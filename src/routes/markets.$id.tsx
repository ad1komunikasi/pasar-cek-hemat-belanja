...
function MarketDetail() {
  ...
  const { data: market } = useQuery({
    queryKey: ["market", id],
    queryFn: async () =>
      (await supabase.from("markets").select("*").eq("id", id).maybeSingle()).data,
  });
              ...
              <>
                ...
                <div className="space-y-2 text-xs text-gray-600">
                  ...
                  <p className="leading-relaxed">
                    Peta di atas menunjukkan lokasi tepat dari <strong>{market.name}</strong>. Anda
                    dapat melihat navigasi langsung dengan mengklik tombol "Buka di Google Maps" di
                    atas.
                  </p>
                </div>
              </>
