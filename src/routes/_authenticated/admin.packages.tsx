...
function AdminPackages() {
  ...
  const [name, setName] = useState(""); const [slug, setSlug] = useState(""); const [price, setPrice] = useState(""); const [days, setDays] = useState(""); const [desc, setDesc] = useState("");

  async function create() {
    if (!name || !slug || !price || !days) return toast.error("Lengkapi semua field");
    const { error } = await supabase.from("packages").insert({ name, slug, price: Number(price), duration_days: Number(days), description: desc, benefits: [] });
    if (error) return toast.error(error.message);
    toast.success("Paket dibuat");
    setName(""); setSlug(""); setPrice(""); setDays(""); setDesc("");
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="overflow-x-auto rounded-lg border border-[var(--color-gray-100)] bg-white">
          <table className="w-full text-sm">
            <thead className="bg-[var(--color-gray-50)] text-left text-xs uppercase text-[var(--color-gray-500)]">
              <tr><th className="px-4 py-3">Nama</th><th className="px-4 py-3">Slug</th><th className="px-4 py-3 text-right">Harga</th><th className="px-4 py-3 text-right">Durasi</th><th className="px-4 py-3">Aktif</th><th /></tr>
            </thead>
            <tbody>
              {(data ?? []).map((p: any) => (
                <tr key={p.id} className="border-t border-[var(--color-gray-100)]">
                  ...
                  <td className="px-4 py-3"><button onClick={() => toggle(p.id, p.is_active)} className={`rounded px-2 py-1 text-xs ${p.is_active ? "bg-[var(--color-success)] text-white" : "bg-[var(--color-gray-100)]"}`}>{p.is_active ? "ON" : "OFF"}</button></td>
        ...
        <div className="space-y-3 rounded-lg border border-[var(--color-gray-100)] bg-white p-4">
          ...
          <div><Label>Harga (IDR)</Label><Input type="number" value={price} onChange={(e) => setPrice(e.target.value)} /></div>
          <div><Label>Durasi (hari)</Label><Input type="number" value={days} onChange={(e) => setDays(e.target.value)} /></div>
