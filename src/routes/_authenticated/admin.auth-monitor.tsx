...
export const Route = createFileRoute("/_authenticated/admin/auth-monitor")({
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          label="Failed Login"
          value={(data?.logs ?? []).filter((l: any) => !l.success).length}
          icon={ShieldAlert}
          accent="danger"
        />
      </div>
          ...
          <tbody>
            {(data?.logs ?? []).map((l: any) => (
              <tr key={l.id} className="border-t border-[var(--color-gray-100)]">
                ...
                <td
                  className="px-4 py-3 truncate text-xs text-[var(--color-gray-500)]"
                  style={{ maxWidth: 360 }}
                >
                  {l.user_agent ?? "—"}
                </td>
