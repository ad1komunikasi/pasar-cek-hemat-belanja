...
function AdminPayments() {
  ...
  const { data: methods } = useQuery({
    queryKey: ["admin-pm"],
    queryFn: async () =>
      (await supabase.from("payment_methods").select("*").order("sort_order")).data ?? [],
  });
    ...
    <>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-[var(--color-gray-500)]">
            Atur rekening bank, VA, QRIS, atau e-wallet tujuan transfer pembayaran dari pengguna.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(methods ?? []).map((m: any) => (
          <div
            key={m.id}
            className={`flex flex-col justify-between rounded-lg border bg-white p-5 shadow-xs transition-all duration-200 ${m.is_active ? "border-[var(--color-gray-100)]" : "border-[var(--color-gray-200)] opacity-70"}`}
          >
            <div>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <h3 className="font-bold text-base text-[var(--color-ink)] leading-tight">
                    {m.name}
                  </h3>
                  <div className="mt-1 flex items-center gap-1.5">
                    ...
                    <span className="text-2xs text-[var(--color-gray-500)]">
                      Sortir: {m.sort_order}
                    </span>
                  </div>
                </div>
                <Badge
                  className={
                    m.is_active
                      ? "bg-[var(--color-success)] border-0 text-white"
                      : "bg-[var(--color-gray-200)] text-[var(--color-gray-600)] border-0 flex items-center gap-1"
                  }
                >
              ...
              {m.account_number && (
                <div className="my-3 rounded-md bg-[var(--color-gray-50)] p-2.5">
                  <p className="text-[10px] uppercase font-bold text-[var(--color-gray-500)]">
                    Rekening / Tujuan
                  </p>
                  ...
                  {m.account_name && (
                    <p className="text-xs text-[var(--color-gray-700)]">a.n. {m.account_name}</p>
                  )}
                </div>
              )}
              ...
              {m.instructions && (
                <div className="mt-2 text-xs text-[var(--color-gray-500)] leading-relaxed">
                  <span className="font-semibold block text-[var(--color-gray-700)]">
                    Instruksi:
                  </span>
            ...
            <div className="mt-5 flex items-center justify-between border-t border-[var(--color-gray-100)] pt-3.5 gap-2">
              <Button
                size="sm"
                variant="ghost"
                className="text-xs font-semibold text-[var(--color-gray-500)]"
              ...
              <div className="flex gap-1">
                <Button
                  size="icon"
                  variant="outline"
                  className="h-8 w-8 text-[var(--color-gray-700)]"
                  onClick={() => openEdit(m)}
                >
                ...
                <Button
                  size="icon"
                  variant="outline"
                  className="h-8 w-8 text-[var(--color-destructive)] hover:bg-[oklch(0.95_0.02_27)] hover:text-[var(--color-destructive)] border-0"
                  onClick={() => remove(m.id)}
                >
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center justify-between border-b border-[var(--color-gray-100)] pb-3">
              ...
              <button
                onClick={() => setModalOpen(false)}
                className="rounded p-1 hover:bg-[var(--color-gray-100)] text-[var(--color-gray-500)]"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  id="pm-name"
                  placeholder="Contoh: Bank BCA, Mandiri Transfer, QRIS"
                  value={name}
              ...
              <div>
                ...
                <Input
                  id="pm-number"
                  placeholder="Contoh: 1234567890 atau link QRIS"
                  value={accountNumber}
              ...
              <div>
                ...
                <Input
                  id="pm-account-name"
                  placeholder="Contoh: PT Pasar Cek Hemat"
                  value={accountName}
              ...
              <div>
                ...
                <Textarea
                  id="pm-instructions"
                  placeholder="Petunjuk khusus seperti 'Harap isi berita transfer dengan no. pesanan'..."
                  value={instructions}
