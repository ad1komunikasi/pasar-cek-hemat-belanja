  ...
  const { data: order } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("orders")
          .select("*, package:packages(*), payment_method:payment_methods(*)")
          .eq("id", id)
          .maybeSingle()
      ).data,
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description={fmtDateTime(order.created_at)}
      />
      ...
      {order.status === "active" || order.status === "paid" ? (
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-[var(--color-success)] bg-[oklch(0.95_0.02_145)] p-4 text-[oklch(0.25_0.05_145)]">
          ...
          <div>
            <h4 className="font-bold text-[oklch(0.20_0.05_145)]">
              Pembayaran Berhasil & Paket Aktif!
            </h4>
            <p className="text-sm">
              Terima kasih atas pembayaran Anda. Akun Anda telah resmi ditingkatkan ke Premium.
              Silakan nikmati semua fitur eksklusif PasarCek.
            </p>
          </div>
        </div>
      ) : order.status === "rejected" ? (
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-[var(--color-destructive)] bg-[oklch(0.95_0.02_27)] p-4 text-[oklch(0.25_0.05_27)]">
          ...
          <div>
            ...
            <p className="text-sm">
              Bukti transfer Anda ditolak dengan alasan:{" "}
              <strong>{order.admin_note || "Data transfer tidak sesuai."}</strong>
            </p>
            <p className="mt-1 text-xs">
              Silakan transfer dengan nominal yang benar dan unggah ulang bukti pembayaran yang sah
              di bawah.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-[var(--color-destructive)] bg-red-50 p-4 text-red-800">
          ...
          <div>
            ...
            <p className="text-sm">
              Waktu 24 jam untuk melakukan pembayaran telah berlalu. Pesanan ini telah dibatalkan
              secara otomatis. Silakan kembali ke halaman penawaran untuk membuat pesanan paket
              baru.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-[oklch(0.78_0.15_75)] bg-[oklch(0.98_0.02_75)] p-4 text-[oklch(0.25_0.05_70)]">
          ...
          <div>
            ...
            <p className="text-sm">
              Rincian tagihan ini telah dikirim ke email Anda:{" "}
              <strong>{order.recipient_email}</strong>. Silakan lakukan transfer sesuai rincian di
              bawah dan unggah bukti pembayaran di halaman ini untuk aktivasi instan.
            </p>
          </div>
        </div>
      ) : order.status === "proof_uploaded" || order.status === "verifying" ? (
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-[var(--color-info)] bg-[oklch(0.95_0.02_258)] p-4 text-[oklch(0.25_0.05_258)]">
          ...
          <div>
            ...
            <p className="text-sm">
              Bukti pembayaran Anda telah kami terima dan sedang diproses oleh admin untuk
              verifikasi manual. Proses ini biasanya memakan waktu 10-60 menit. Halaman ini akan
              diperbarui otomatis saat status berubah.
            </p>
          </div>
        </div>
      ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center text-center">
                    <span
                      className={`mb-2 flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${colorClass}`}
                    >
                      {i + 1}
                    </span>
            ...
            <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-6 shadow-sm">
              <div className="mb-4 flex flex-col justify-between gap-3 border-b border-[var(--color-gray-100)] pb-4 sm:flex-row sm:items-center">
                <div>
                  ...
                  <p className="text-xs text-[var(--color-gray-500)]">
                    Harap selesaikan pembayaran sebelum batas waktu berakhir
                  </p>
                </div>
                {timeLeft !== null && (
              ...
              <div className="mb-4 rounded-md bg-[var(--color-gray-50)] p-4 text-sm space-y-3">
                ...
                <Row
                  ...
                  value={fmtDateTime(getDeadlineDate().toISOString())}
                />
              </div>

              {m.instructions && (
                <div className="rounded-md border border-dashed border-[var(--color-gray-300)] bg-white p-4">
                  <span className="text-xs font-bold uppercase text-[var(--color-gray-500)] block mb-1">
                    Catatan Tambahan:
                  </span>
                  <p className="text-sm text-[var(--color-gray-700)] leading-relaxed whitespace-pre-wrap">
                    {m.instructions}
                  </p>
                </div>
              )}
            ...
            <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="flex gap-3">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[oklch(0.92_0.02_195)] text-xs font-bold text-[oklch(0.48_0.08_195)]">
                    1
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-[var(--color-gray-500)]">
                      ...
                      <strong className="font-mono text-[var(--color-ink)]">
                        {idr(Number(order.amount))}
                      </strong>{" "}
                      ke rekening di atas menggunakan ATM, m-banking, atau internet banking Anda.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--color-gray-500)]">
                      Ambil screenshot atau simpan struk transfer pembayaran Anda yang menunjukkan
                      nomor transaksi sukses.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--color-gray-500)]">
                      Pilih file bukti pembayaran Anda di form di bawah ini dan klik tombol Unggah.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--color-gray-500)]">
                      Admin akan melakukan verifikasi pembayaran. Setelah diverifikasi, paket Anda
                      akan langsung diaktifkan secara otomatis.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {(order.status === "pending_payment" || order.status === "rejected") && !isExpired && (
            <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-6 shadow-sm">
              ...
              <p className="mb-4 text-xs text-[var(--color-gray-500)]">
                Pilih file bukti pembayaran Anda di bawah ini, lalu klik tombol kirim bukti
                transfer.
              </p>
                ...
                <div className="space-y-4">
                  <div className="rounded-md border border-[var(--color-gray-100)] bg-[var(--color-gray-50)] p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-sm truncate text-[var(--color-ink)]">
                        {selectedFile.name}
                      </p>
                      <p className="text-xs text-[var(--color-gray-500)]">
                  ...
                  {selectedFile.type.startsWith("image/") && (
                    <div className="mt-3 overflow-hidden rounded-md border border-[var(--color-gray-100)] bg-[var(--color-gray-50)] max-h-[220px] flex justify-center p-2">
                      <img
                        ...
                        className="max-h-[200px] object-contain rounded shadow-xs"
                      />
                    </div>
                  )}
          ...
          {order.proof_url && (
            <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between gap-3 rounded-md bg-[var(--color-gray-50)] p-3 text-sm">
                <span className="truncate text-xs font-mono text-[var(--color-gray-500)]">
                  File: {order.proof_url.split("/").pop()}
                </span>
                <Badge variant="outline" className="bg-[var(--color-success)] text-white border-0">
                  Sudah Diupload
                </Badge>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-6 shadow-sm">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--color-gray-500)] mb-3">
              Ringkasan Pesanan
            </h3>
            <div className="text-3xl font-black mb-4 text-[var(--color-ink)]">
              {idr(Number(order.amount))}
            </div>
