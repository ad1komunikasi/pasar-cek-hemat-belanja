...
function SmartBasketPage() {
  ...
  const { data: basket } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: created } = await supabase
        .from("smart_baskets")
        .insert({ user_id: user!.id, name: "Keranjang Saya" })
        .select()
        .single();
      return created!;
    },
  ...
  const recommendations = useMemo(() => {
    ...
    const markets = new Map<
      string,
      { id: string; name: string; city: string; total: number; covered: number }
    >();
    ...
    return Array.from(markets.values())
      .filter((m) => m.covered === items.length)
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-[var(--color-gray-50)] p-4 border border-[var(--color-gray-100)] text-xs text-[var(--color-gray-700)]">
        ...
        <div className="flex items-center gap-1.5 font-medium">
          <span className="relative flex h-2 w-2">
            ...
            <span
              className={`relative inline-flex rounded-full h-2 w-2 ${selectedDate === today ? "bg-[var(--color-success)]" : "bg-amber-500"}`}
            ></span>
          </span>
          {selectedDate === today ? (
            <span>
        ...
        <div className="rounded-lg border border-[var(--color-gray-100)] bg-white @container">
          <div className="border-b border-[var(--color-gray-100)] p-4">
            <div className="grid gap-2 sm:grid-cols-[1fr_120px_auto]">
              ...
              <Input
                ...
                placeholder="Jumlah"
              />
              <Button onClick={addItem} disabled={!newProduct}>
          ...
          {!items || items.length === 0 ? (
            <EmptyState
              ...
              description="Tambahkan produk untuk mulai menghitung penghematan."
            />
          ) : (
                    ...
                    >
                      ...
                      <div className="border-t border-border/60 pt-3 space-y-2">
                        ...
                        {productPrices.length === 0 ? (
                          <div className="text-[10px] text-[var(--color-gray-500)] italic py-1">
                            Tidak ada data harga hari ini
                          </div>
                        ) : (
                          productPrices.map((row: any, idx: number) => {
                            ...
                            if (isCheapest) {
                              return (
                                <div
                                  key={row.market.id}
                                  className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded-xl bg-accent/5 border border-accent/10 text-foreground"
                                >
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <div className="flex flex-col items-end">
                                      <span className="font-bold text-primary">
                                        {idr(Number(row.price))}
                                      </span>
                                      <span className="text-[9px] text-[var(--color-gray-500)] font-medium">
                            ...
                            if (!isPremium) {
                              return (
                                <div
                                  key={row.market.id}
                                  className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded-xl bg-slate-50/50 border border-slate-100/30 text-foreground/45 relative overflow-hidden select-none"
                                >
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span
                                      ...
                                      onClick={() =>
                                        handleOpenLock("Smart Basket Lengkap (Detail Pasar)")
                                      }
                                    >
                            ...
                            return (
                              <div
                                key={row.market.id}
                                className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded-xl bg-transparent text-foreground/80"
                              >
                                <div className="flex flex-col">
                                  <span>{row.market.name}</span>
                                ...
                                <div className="flex flex-col items-end">
                                  ...
                                  <span className="text-[9px] text-[var(--color-gray-400)] font-medium">
                                    Total: {idr(totalMarketPrice)}
                                  </span>
                                </div>
                              </div>
              ...
              <div className="mt-4 flex flex-col @md:flex-row @md:items-center justify-between gap-4 rounded-xl bg-[var(--color-gray-50)] p-4 border-2 border-dashed border-[var(--color-gray-200)]">
                <div>
                  ...
                  <p className="text-[10px] text-[var(--color-gray-500)]">
                    Kombinasi harga termurah untuk setiap produk dari berbagai pasar.
                  </p>
                </div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-gradient-to-br from-[var(--color-brand-blue)] to-[var(--color-brand-green)] p-6 text-white shadow-soft">
            ...
            <p className="mt-1 text-xs text-white/80">
              Selisih antara pasar termurah & termahal di keranjang Anda
            </p>
          </div>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-md bg-[var(--color-gray-50)] p-3 border border-[var(--color-gray-100)] hover:border-[var(--color-brand-blue)]/30 transition-all">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-brand-blue)] bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded">
                        Taktik A: Satu Lokasi
                      </span>
                ...
                <div className="relative rounded-md bg-[var(--color-brand-green)]/[0.03] p-3 border border-[var(--color-brand-green)]/15 overflow-hidden">
                  {!isPremium && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/80 backdrop-blur-xs p-3 text-center">
                      ...
                      <p className="text-[9px] text-[var(--color-gray-700)] font-bold">
                        Taktik B: Lintas Pasar Terkunci
                      </p>
                      <button
                        onClick={() => handleOpenLock("Smart Basket Lengkap (Taktik Multi-Pasar)")}
                        className="mt-1 text-[9px] text-[var(--color-brand-blue)] font-extrabold underline hover:text-[var(--color-brand-green)]"
                      >
                        Buka Sekarang
                      </button>
                    </div>
                  ...
                  <div
                    className={
                      !isPremium ? "blur-[1.5px] opacity-35 select-none pointer-events-none" : ""
                    }
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-brand-green)] bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">
                          Taktik B: Multi-Pasar
                        </span>
                    ...
                    <p className="text-[10px] text-[var(--color-brand-green)] mt-1.5 leading-relaxed">
                      Beli tiap produk di pasar termurahnya masing-masing untuk mendapatkan total
                      harga paling murah.
                    </p>
                  </div>
                </div>
                ...
                {cheapest && crossMarketTotal > 0 && cheapest.total > crossMarketTotal ? (
                  <div className="flex items-start gap-2 rounded-md bg-[var(--color-success)]/5 p-3 border border-[var(--color-success)]/15 text-[var(--color-success)] text-[11px] leading-relaxed">
                    ...
                    <span>
                      ...
                      <strong className="font-bold text-xs">
                        {idr(cheapest.total - crossMarketTotal)}
                      </strong>{" "}
                  ...
                  <div className="flex items-start gap-2 rounded-md bg-blue-50/50 p-2.5 border border-blue-100 text-[var(--color-brand-blue)] text-[10px]">
                    ...
                    <span>
                      Harga satu pasar termurah sudah optimal! Belanja di {cheapest.name} memberikan
                      harga terbaik tanpa perlu mengunjungi beberapa pasar.
                    </span>
                  </div>
                ) : null}
              </div>
          ...
          <div className="rounded-lg border border-[var(--color-gray-100)] bg-white p-4 shadow-soft">
            <h3 className="mb-3 text-sm font-bold uppercase text-[var(--color-gray-500)]">
              Rekomendasi Pasar
            </h3>
            {recommendations.length === 0 ? (
              <p className="text-sm text-[var(--color-gray-500)]">
                Tambahkan produk untuk melihat rekomendasi.
              </p>
            ) : (
              <ul className="space-y-2">
                {recommendations.map((m, i) => (
                  <li
                    key={m.id}
                    className={`flex items-center justify-between rounded-md border p-3 transition-colors ${i === 0 ? "border-[var(--color-success)] bg-[var(--color-success)]/5" : "border-[var(--color-gray-100)] hover:bg-[var(--color-gray-50)]/50"}`}
                  >
                    <div>
                      <p className="flex items-center gap-2 font-semibold text-xs sm:text-sm">
                        {i === 0 && <Trophy className="h-4 w-4 text-[var(--color-success)]" />}
                        {m.name}
                      </p>
                      <p className="text-[10px] text-[var(--color-gray-500)]">{m.city}</p>
                    </div>
                    <p
                      className={`text-sm sm:text-base font-black ${i === 0 ? "text-[var(--color-success)]" : ""}`}
                    >
                      {idr(m.total)}
                    </p>
                  </li>
