...
function CheckoutPage() {
  ...
  const { data: pkg } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("packages").select("*").eq("slug", search.package).maybeSingle()).data,
  });
  const { data: methods } = useQuery({
    queryKey: ["payment-methods"],
    queryFn: async () =>
      (await supabase.from("payment_methods").select("*").eq("is_active", true).order("sort_order"))
        .data ?? [],
  });
  ...
  async function submit() {
    ...
    const num =
      (numRes as any) ??
      `PSC-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${Math.floor(1000 + Math.random() * 8999)}`;
    ...
    toast.success(
      `Pesanan berhasil dibuat! Rincian instruksi transfer telah dikirim ke email ${email}.`,
    );
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-3 rounded-lg border border-[var(--color-gray-100)] bg-white p-6">
          ...
          <div className="flex justify-between text-sm">
            <span>{pkg.name}</span>
            <span>{idr(Number(pkg.price))}</span>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-xl font-black">{idr(Number(pkg.price))}</span>
          </div>
        </div>
      </div>
    </AppShell>
